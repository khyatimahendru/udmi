...
public class TargetProcessor extends ProcessorBase {
  ...
  private void processKeyRotation(Envelope envelope, SystemEvents systemEvents) {
    String registryId = envelope.deviceRegistryId;
    String deviceId = envelope.deviceId;
    info("Processing key rotation for %s/%s", registryId, deviceId);
    CloudModel modelUpdate = new CloudModel();
    modelUpdate.operation = ModelOperation.UPDATE;
    modelUpdate.credentials = systemEvents.credentials;
    iotAccess.modelDevice(registryId, deviceId, modelUpdate, null);
  }
